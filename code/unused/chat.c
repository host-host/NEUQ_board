...
__attribute((constructor)) void chat_init(){
    ...
    ldata=lseek(fdata=open("/web/res/pri/data.dat",O_RDWR|O_APPEND|O_CREAT,S_IRUSR|S_IWUSR),0,SEEK_END);
    data=(char*)mmap(0,1ll<<30,PROT_READ|PROT_WRITE,MAP_SHARED,fdata,0);
    lcont=lseek(fcont=open("/web/res/pri/cont.dat",O_RDWR|O_APPEND|O_CREAT,S_IRUSR|S_IWUSR),0,SEEK_END);
    cont=(char*)mmap(0,1ll<<30,PROT_READ|PROT_WRITE,MAP_SHARED,fcont,0);
}
static ll readll(const char* a){
    ll x=0;
	while(*a!='-'&&(*a<'0'||*a>'9'))a++;
	if(*a=='-') {
...
static void myJSON(const char* p,char* a){
    int i=0,bj=0;
    if(p[0]=='\"')p++;
    a[i++]='\"';
    while(*p){
